...
extern unsigned long long gpgs_per_line;
extern unsigned long long gtt_pgs;
int modeling_num = -1;
...
/*initialize model*/
void model_create(int write_size, struct ssd *ssd) {
	...
	if (write_size == 0)
		mmodel->time_window=44000*1024/(gpgs_per_line)*1024/4/mmodel->interval_unit_size; //800GB/(PPB*BPS)
	else
		mmodel->time_window = write_size*1024/(gpgs_per_line)*1024/4/mmodel->interval_unit_size;
	...
	*/
	...
	if (mmodel->lba_sampling_ratio > 1) {
		...
		q_init(&(mmodel->fqueue), gpgs_per_line*mmodel->interval_unit_size);
		mmodel->live_lba=0;
	...
	printf("- UID interval unit size: %d segments (%lld pages)\n", mmodel->interval_unit_size, mmodel->interval_unit_size*(gpgs_per_line));
	printf("- Epoch size: %.2fGB (%llu units)\n", mmodel->time_window/1024.*(gpgs_per_line)/1024.*mmodel->interval_unit_size*4, mmodel->time_window);
	// sleep(3);
	...
	mmodel->time_stamp = (unsigned long long*)malloc(sizeof(unsigned long long)*gtt_pgs/mmodel->lba_sampling_ratio);
	mmodel->hot_lba = (int*)malloc(sizeof(int)*gtt_pgs/mmodel->lba_sampling_ratio);
	for (int i=0; i<gtt_pgs/mmodel->lba_sampling_ratio; i++){
...
void time_managing(char mode) {
	...
	mm_time.request_time++;
        if (mm_time.request_time == (gpgs_per_line)*mm_time.interval_unit_size) {
...
int check_time_window(struct ssd *ssd, uint64_t lba, char mode) {
	...
	/* time update */
		...
		if (mm_time.load_timing < (unsigned long)gtt_pgs+mm_time.extra_load) {
			mm_time.load_timing++;
		}
                else {
	...
	check_interval(ssd, lba, M_WRITE);
	return 1;
}

/* checking lba's interval */
int check_interval(struct ssd *ssd, uint64_t lba, char mode) {
	...
	*/
	...
	if (mm_time.current_time >= mm_time.time_window) {
		...
		modeling_num += 1;
		int status = pthread_create(&mmodel->thread_id, NULL, making_group_configuration, (void*)ssd);
		if (status != 0) {
...
int update_count(uint64_t lba, char mode) {
	//TODO
...
void *making_group_configuration(void *arg) {
	...
	uint32_t **opt_config = (uint32_t**)malloc(sizeof(uint32_t*)*10);
        ...
        opt_config[0][0] = ssd->sp.tt_lines - ssd->sp.gc_thres_lines_high;
	...
	if (hot_q->is_fix==true) {
		...
		for (int i=1;i<30;i++) {
			...
			group_config[0] = ssd->sp.tt_lines - ssd->sp.gc_thres_lines_high - i;
			hot_q->calc_unit=0;
...
double WAF_predictor(double *valid_ratio_list, int group_num) {
	...
	base[0]=gpgs_per_line*0.1;
	base[1]=gpgs_per_line*0.9;
...
unsigned long long resizing_model(void) {
...
//	printf("Un-uniform FIFO prediction, tot_req_count: %lld, hot_req_count: %lld, op: %.2f, r: %.4f, f: %.4f, hot_lba_count: %.4f, tot_lba: %.4f\n", tot_req_count, hot_req_count, op, r, f, hot_lba_count, tot_lba_count);
	...
	//int lines = 0;
	while(test_val > 0){
		test_val = 1 + r/(double)(exp((r*op)/(f*inc_waf))-1) + (1-r)/(double)(exp(((1-r)*op)/((1-f)*inc_waf))-1)-inc_waf;
		inc_waf += 1;
	//	lines += 1;
/*		if (inc_waf < 20) {
...
// double one_group_predictor(uint32_t *group_config, uint32_t group_num, unsigned long long tot_cnt,struct ssd *ssd) {	//input values are same as vlid_ratio_predictor, return valid ratio
// 	double last_vr;
// 	double last_group_vp = (double)utilization;  //global var
// 	//printf("op: %.4f\n", (double) last_group_vp/((double)group_config[group_num-1]*(_PPS*L2PGAP)));
// 	double last_waf = None_FIFO_predict((double)group_config[group_num-1]*(gpgs_per_line)/(double)last_group_vp);	//None_FIFO_predict()'s input is op 
...
// 		last_waf = None_FIFO_predict((1+1/double(2*(gpgs_per_line)))*(double)group_config[group_num-1]*(gpgs_per_line)/(double)last_group_vp)/(1+1/double(2*(gpgs_per_line)));	//Changed op is input
// 		last_vr = 1.-1./last_waf;
// 		return last_waf;
// 	}
// 	else{
...
// 			}
// 		}
// 		if (i==0) group_time = (double)group_config[i]*(double)(gpgs_per_line)*mig_rate-(gpgs_per_line)/2;
// 		else group_time = (double)group_config[i]*(double)(gpgs_per_line)*mig_rate-(gpgs_per_line)*mig_rate/2;
// 		//error check: if number of segments of groups over the total time window = time window < device size
// 		if ((i!=group_num-1) && (((uint32_t)group_time+past_group_time) >= ((mmodel->time_window*mm_time.interval_unit_size-50)*(gpgs_per_line)))) {
...
// 			cur_interval = (double)(j)*(double)(gpgs_per_line)*(double)mm_time.interval_unit_size;
...
// 					global_valid_cnt += tmp_vr*ref_rate*(gpgs_per_line)*mm_time.interval_unit_size;
...
//                                         double lba_num = (double) mmodel->model_count[j] / ((double) ((mmodel->time_window*mm_time.interval_unit_size-50)*(gpgs_per_line)) / (double) cur_interval); //count lba that has lifespan as cur_interval
...
// 	double last_group_vr = last_group_vp/(double)((group_config[group_num-1])*(gpgs_per_line));
...
// 	double last_waf = None_FIFO_predict((double)group_config[group_num-1]*(gpgs_per_line)/(double)last_group_vp);
// 	double last_vr;
// 	last_vr = 1.-1./last_waf;
...
// 	}
// 	if (DES == 2){ // Greedy
// 		last_waf = None_FIFO_predict((1+1/(double)(2*(gpgs_per_line)))*(double)group_config[group_num-1]*(gpgs_per_line)/(double)last_group_vp)/(1+1/(double)(2*(gpgs_per_line)));
// 		last_vr = 1.-1./last_waf;
// 		valid_ratio_list[group_num-1] = last_vr;
// 		goto END;
// 	}
...
double *hot_valid_ratio_predictor(uint32_t *group_config, uint32_t group_num, unsigned long long tot_cnt, int hot_size) {
	...
	if (hot_q->is_fix) {
		...
		for (int i=0;i<desired_idx;i++) {
			...
			global_valid_cnt += tmp_vr*ref_rate*(double)mm_time.interval_unit_size*(double)(gpgs_per_line);
		}
		...
		if (hot_q->calc_unit==0) {
			for (int i=0;i<mmodel->entry_num;i++) {
				...
				global_valid_cnt += tmp_vr*(double)mm_time.interval_unit_size*(double)(gpgs_per_line);
	...
	//printf("last_group_vr: %f, (", last_group_vr);
	if (last_group_vp < 0) {
		printf("last group vr is over 1\nlast-group vr: %.2f, spare space: %lld\n", last_group_vp,(group_config[group_num-1])*(gpgs_per_line));
		//printf("group confi: %d\n", group_config[group_num-1]);
		free(valid_ratio_list);
		free(invalid_cnt_list);
		free(last_vr_list);
...
void initialize_first_interval(void) {
...
void *first_interval_analyzer(void* arg) {
	...
	uint32_t one_unit = gpgs_per_line*mm_time.interval_unit_size;
...
void remove_first_interval(void) {
